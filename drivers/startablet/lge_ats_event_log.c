...
static void ats_event_log_event(struct input_handle *handle, unsigned int type,unsigned int code, int value)
{
	...
	// yooseok.shim@lge.com | BEGIN
	// Remove Tiny Movement
            /*
            if (ats_mtc_key_log1.action == (unsigned char)ETA_TOUCH_MOVETO && move_x < 7 && move_y < 7)
            {
            	printk("yooseok.shim | ETA_TOUCH_MOVETO, tiny movement [%04d][%04d]\n", move_x, move_y);
                ; // ignore tiny move
            }
            else
            */
            // yooseok.shim@lge.com | END
            {
            	printk("yooseok.shim | Move!!\n");
