...
struct spi_tegra_data {
	...
	unsigned (*spi_tegra_tx)(struct spi_tegra_data *tspi,
				  struct spi_transfer *t);
	u8 g_bits_per_word;
    struct work_struct       work_spi;
    struct work_queue_struct *wq_spi;
	struct semaphore sem_complete ;
};

static struct task_struct *write_task;

// static void spi_tegra_complete(struct work_struct *work) ;
static int spi_tegra_complete(void *tspi_ptr);
static int spi_tegra_cs_setup(struct spi_device *spi);
...
static void complete_operation(struct spi_tegra_data *tspi)
{
	unsigned long val;
	struct spi_message *m;
	...
	*/
	...
	while ((spi_tegra_readl(tspi, SLINK_STATUS) & SLINK_BSY)) {
		if (timeout++ > 1000){
			printk("SLINK_BSY timeout \n") ;
			break;
		}
	}
	while ((spi_tegra_readl(tspi, SLINK_STATUS2)) != SLINK_STATUS2_RESET) {
		if (temp++ > 50000){
			val = spi_tegra_readl(tspi, SLINK_STATUS2) ;
			printk("SLINK_STATUS2_RESET timeout 0x%08x 0x%08x \n", val,spi_tegra_readl(tspi, SLINK_STATUS) ) ;
			break;
		}
	}
	...
	if (!list_is_last(&tspi->cur->transfer_list, &m->transfers)) {
		...
		printk("spi_tegra_start_transfer in complete_operation \n") ;
	} else {
		list_del(&m->queue);
		...
		if (!list_empty(&tspi->queue)) {
			...
			printk("spi_tegra_start_message in complete_operation \n") ;
		} else {
...
{
	// printk("%s \n", __FUNCTION__);
	struct spi_tegra_data *tspi = (struct spi_tegra_data *)tspi_ptr;

	while(!kthread_should_stop()){
        down(&tspi->sem_complete);
		complete_operation(tspi);
    }
...
static void spi_tegra_complete(struct work_struct *work)
{
	struct spi_tegra_data *tspi = container_of(work, struct spi_tegra_data, work_spi);
	complete_operation(tspi);
}
#endif
...
/*
	if (((tspi->rx_complete) == 1) && ((tspi->tx_complete) == 1)){
		queue_work(tspi->wq_spi, &tspi->work_spi);
		if(!work_inqueue)
			printk(">>>>>-----^-^------>>>>> \n") ;
	}
*/
	if (((tspi->rx_complete) == 1) && ((tspi->tx_complete) == 1)){
		up(&(tspi->sem_complete)) ;
	}
...
static int __init spi_tegra_probe(struct platform_device *pdev)
{
	...
	sema_init(&(tspi->sem_complete) , 0) ;

	if (ret < 0)
		goto err7;

	tspi->wq_spi = create_singlethread_workqueue("spi_tegra");
	if(!tspi->wq_spi){
		printk("Failed to setup workqueue - spi_tegra \n");
    }

	// INIT_WORK(&tspi->work_spi,spi_tegra_complete);
	printk("%s \n", __FUNCTION__) ;

	write_task = kthread_run(spi_tegra_complete, tspi, "spi_complete") ;
...
static int spi_tegra_resume(struct platform_device *pdev)
{
	...
	//	spi_tegra_writel(tspi, tspi->save_slink_cmd, SLINK_COMMAND);
		dumdy_read = spi_tegra_readl(tspi, SLINK_COMMAND);
		clk_disable(tspi->clk);
		tspi->cur_speed = 0;
		tspi->is_suspended = false;
		spin_unlock_irqrestore(&tspi->lock, flags);
