...
/*
* Control LDO for USB driver
*/
static int control_udc_ldo(int enable)
{
	struct regulator* udc_regul;
	int ret = 0;

	// get reulator
	udc_regul = regulator_get(NULL, "avdd_usb");
	if (IS_ERR(udc_regul))
	{
		printk(KERN_INFO "avdd_usb regulator found\n");
		return -1;
	}

	if (enable)
	{
		ret = regulator_enable(udc_regul);
	}
	else
	{
		ret = regulator_disable(udc_regul);
	}
