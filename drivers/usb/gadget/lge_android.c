...
#ifdef CONFIG_USB_ANDROID_LGE_GADGET
int gserial_count = 0;
extern int usb_modem_mode;
extern void update_mtp_ext_config_desc(int index, int is_iad, int is_mtp);
#endif
...
#ifdef CONFIG_USB_ANDROID_LGE_GADGET
	if(gserial_count)
		gserial_setup(dev->config->cdev->gadget, gserial_count);
#endif
}
...
static int android_bind(struct usb_composite_dev *cdev)
{
	...
	dev->cdev = cdev;
#ifdef CONFIG_USB_ANDROID_LGE_GADGET
	if (device_desc.idProduct == 0x6000) {
...
void update_dev_desc(struct android_dev *dev)
{
	...
	list_for_each_entry(f, &android_config_driver.functions, list) {
		if (!f->disabled) {
#ifdef CONFIG_USB_ANDROID_LGE_GADGET
			update_mtp_ext_config_desc(num_enabled,
					f->descriptors[0]->bDescriptorType == USB_DT_INTERFACE_ASSOCIATION,
					strcmp(f->name, "mtp") == 0);
#endif
			num_enabled++;
			last_enabled_f = f;
...
#ifndef CONFIG_USB_ANDROID_LGE_GADGET
			break;
#endif
		}
	}
...
#ifdef CONFIG_USB_ANDROID_LGE_GADGET
static int product_matches_enabled_function(struct android_usb_product *p, char *name, int enabled)
{
	int matched = 0;
	char **functions = p->functions;
	int count = p->num_functions;
...
static int get_product_array_index(struct usb_function *f, int enable)
{
	int i;
	static int adb_enabled, rndis_enabled = 0;
	struct android_dev *dev = _android_dev;
	struct android_usb_product *p = dev->products;
	...
	if(!strcmp(f->name, "adb"))
		...
		rndis_enabled = enable;

	for (i = 0; i < dev->num_products; i++, p++) {
		if (enable) {
			if (!product_matches_enabled_function(p, "adb", adb_enabled))
				continue;

			if (!product_matches_enabled_function(p, "rndis", rndis_enabled))
				continue;
			...
			if (product_matches_enabled_function(p, "adb", adb_enabled)
				&& product_matches_enabled_function(p, "rndis", rndis_enabled))
				return i;
		}
	}
	return 0;
...
void android_enable_function(struct usb_function *f, int enable)
{
	...
	if (!!f->disabled != disable) {
		...
		if (!strcmp(f->name, "rndis") || !strcmp(f->name, "adb"))
		{
			...
			index = get_product_array_index(f, enable);
			num_functions = dev->products[index].num_functions;
			functions = dev->products[index].functions;
			product_id = dev->products[index].product_id;
			...
			list_for_each_entry(func, &android_config_driver.functions, list) {
				...
				{
					if (!strcmp(func->name, functions[i])) {
						found = 1;
					}
				}
				printk("             %s : %d\n", func->name, found);
				usb_function_set_enabled(func, found);
			}
