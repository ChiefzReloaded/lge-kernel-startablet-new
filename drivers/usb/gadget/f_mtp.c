...
struct {
	...
	struct mtp_ext_config_desc_function    function[8];
} mtp_ext_config_desc = {
	.header = {
		.dwLength = __constant_cpu_to_le32(sizeof(struct mtp_ext_config_desc_header) + sizeof(struct mtp_ext_config_desc_function)),
...
#ifdef CONFIG_USB_ANDROID_LGE_GADGET
static int w_index_mtp = 0;

void update_mtp_ext_config_desc(int index, int is_iad, int is_mtp)
{
	static int interfaces = 0;

	if(index == 0) {
		interfaces = 0;
		mtp_ext_config_desc.header.bCount = 0;
		mtp_ext_config_desc.header.dwLength = sizeof(struct mtp_ext_config_desc_header);
	}

	mtp_ext_config_desc.function[index].bFirstInterfaceNumber = interfaces;

	if(is_iad) {
		interfaces++;
		mtp_ext_config_desc.function[index].bInterfaceCount = 2;
	} else {
		mtp_ext_config_desc.function[index].bInterfaceCount = 1;
	}

	if(is_mtp) {
		w_index_mtp = interfaces;
		strcpy(mtp_ext_config_desc.function[index].compatibleID, "MTP");
	} else {
		memset(mtp_ext_config_desc.function[index].compatibleID, 0, sizeof(mtp_ext_config_desc.function[index].compatibleID));
	}

	mtp_ext_config_desc.header.bCount++;
	mtp_ext_config_desc.header.dwLength += sizeof(struct mtp_ext_config_desc_function);
	interfaces++;
}
#endif
					...
					const struct usb_ctrlrequest *ctrl)
{
...
#ifdef CONFIG_USB_ANDROID_LGE_GADGET
	if((ctrl->bRequest >= MTP_REQ_CANCEL && ctrl->bRequest <= MTP_REQ_GET_DEVICE_STATUS)
		&& w_index_mtp == w_index) {
			w_index = 0;
	}
#endif
	...
	if ((ctrl->bRequestType & USB_TYPE_MASK) == USB_TYPE_VENDOR) {
		...
		if (dev->interface_mode == MTP_INTERFACE_MODE_MTP
				...
				&& (w_index == 4 || w_index == 5)) {
#ifdef CONFIG_USB_ANDROID_LGE_GADGET
			value = (w_length < mtp_ext_config_desc.header.dwLength ?
					w_length : mtp_ext_config_desc.header.dwLength);
#else
