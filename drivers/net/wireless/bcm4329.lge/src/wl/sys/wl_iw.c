...
/* LGE_CHANGE_S [jisung.yang@lge.com] 2011-01-20, Do not excute this command. This prevent packet filter */
static int
wl_iw_set_suspend(
struct net_device *dev,
struct iw_request_info *info,
...
/* LGE_CHANGE_E [jisung.yang@lge.com] 2011-01-20, Do not excute this command. This prevent packet filter */
...
#if defined(CONFIG_LGE_BCM432X_PATCH) && defined(SOFTAP)        /*by sjpark 10-12-03 */
int wl_iw_softap_on(struct net_device *dev);
#endif
...
#ifdef SOFTAP
	ap_cfg_running = FALSE;
#if defined(CONFIG_LGE_BCM432X_PATCH)        /*by sjpark 10-12-03 */
        ap_fw_loaded = FALSE;           //by sjpark 10-12-03
...
)
{
	...
	if (g_onoff == G_WLAN_SET_OFF) {
#if defined(CONFIG_LGE_BCM432X_PATCH) && defined(SOFTAP)        /*by sjpark 10-12-03*/
                if(ap_fw_loaded)                //by sjpark 10-12-03
                {
                        wl_iw_softap_on(dev);
                }else
...
int
wl_iw_softap_on(struct net_device *dev)
{
        int ret = 0;
	...
	struct iw_request_info *info, wl_bss_info_t *bi)
{
	...
	char *event;
	//char *buf;
	char *buf = NULL;		//by sjpark 11-01-25 WBT : ID 196408
...
/* LGE_CHANGE_S, [dongp.kim@lge.com], 2011-01-29, when wifi suspend/resume, firmware is not reloaded each time*/
#if defined(CONFIG_LGE_BCM432X_PATCH)
	wl_iw_softap_off(dev,info);             //by sjpark 10-12-03
#endif	/* defined(CONFIG_LGE_BCM432X_PATCH) */
/* LGE_CHANGE_E, [dongp.kim@lge.com], 2011-01-29, when wifi suspend/resume, firmware is not reloaded each time*/
//END: 0015047: [Wi-Fi] when in suspend/resume, wi-fi does not suspend/resume
...
wl_iw_event(struct net_device *dev, wl_event_msg_t *e, void* data)
{
	...
	case WLC_E_NDIS_LINK:
		...
		g_ss_cache_ctrl.m_link_down = 1;
			...
			bzero(wrqu.addr.sa_data, ETHER_ADDR_LEN);
			bzero(&extra, ETHER_ADDR_LEN);
//ADD: 0014488: [Wi-Fi] reduce power consumption by Wi-Fi as long as LCD off
/* LGE_CHANGE_S, [dongp.kim@lge.com], 2011-01-20, reduce power consumption by adaptively setting dtim value as long as LCD off */
			WAKE_LOCK_TIMEOUT(iw->pub, WAKE_LOCK_LINK_DOWN_TMOUT, 5 * HZ);
			//WAKE_LOCK_TIMEOUT(iw->pub, WAKE_LOCK_LINK_DOWN_TMOUT, 20 * HZ);
/* LGE_CHANGE_E, [dongp.kim@lge.com], 2011-01-20, reduce power consumption by adaptively setting dtim value as long as LCD off */
//END: 0014488: [Wi-Fi] reduce power consumption by Wi-Fi as long as LCD off
