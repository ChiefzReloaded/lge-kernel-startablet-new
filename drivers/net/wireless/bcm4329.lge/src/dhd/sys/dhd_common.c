...
#define NV_WIFI_MACADDR "/proc/nvdata/WIFI_MACADDR"
...
/* LGE_CHANGE_S, [jisung.yang@lge.com], 2010-05-27, <Disable setting power save mode if PM is 0> */
		if (var_int == 0) {
			printk("dhd_preinit_proc: do not control power save mode (%d)\n", var_int);
			PM_control = FALSE;
		}
		else {
			printk("dhd_preinit_proc: docontrol power save mode (%d)\n", var_int);
			PM_control = TRUE;
		}
/* LGE_CHANGE_E, [jisung.yang@lge.com], 2010-05-27, <Disable setting power save mode if PM is 0> */
