...
#include <wl_iw.h>
#if defined(CONFIG_LGE_BCM432X_PATCH)
extern void stop_wl_cotrol(void);
#endif
...
dhd_module_cleanup(void)
{
	...
	if(g_net_dev)
	{
		dhd = *(dhd_info_t **)netdev_priv(g_net_dev);
		//if (dhd->sysioc_pid >= 0) {
		if(dhd->thr_sysioc_ctl.thr_pid >= 0){		//by sjpark 11-05-19
			//KILL_PROC(dhd->sysioc_pid, SIGTERM);
			//wait_for_completion(&dhd->sysioc_exited);
			PROC_STOP(&dhd->thr_sysioc_ctl);	//by sjpark 11-05-19
		}
	}
	else
		printk("net device pointer is NULL!!\n");
#endif
#endif
