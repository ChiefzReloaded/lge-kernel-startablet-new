...
int dhd_deep_sleep(struct net_device *dev, int flag)
{
	...
	case 1: /* DEEPSLEEP ON*/
		   ...
		   ret = dhdcdc_set_ioctl(dhd_pub, 0, WLC_SET_VAR, iovbuf, sizeof(iovbuf));
		   if( ret < 0 )
				printk("%s : mpc set fail!!! ret [%d]\n",__func__);
		   ...
		   ret =dhdcdc_set_ioctl(dhd_pub, 0, WLC_SET_VAR, iovbuf, sizeof(iovbuf));
		   if( ret < 0 )
				printk("%s : deepsleep set fail!!! ret [%d]\n",__func__);
		   break;
	...
	case 0: /*DEEPSLEEP OFF*/
		   ...
		   if( ret < 0 )
				...
				ret = dhdcdc_query_ioctl(dhd_pub, 0, WLC_GET_VAR, iovbuf, sizeof(iovbuf));
				if( ret < 0 )
					printk("%s : deepsleep get fail!!! ret [%d]\n",__func__);
				deep_sleep = iovbuf[0];

		   if ((deep_sleep == 1) || (ret <0)) {
				printk("[DEBUG] %s : deepsleep set [%d] to 0.\n",__func__,deep_sleep);
