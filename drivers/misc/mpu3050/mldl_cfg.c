...
static int dmp_write_done=0;
extern int gyro_enabled;
extern int gyro_bypassed;
...
{
    unsigned char fifoBuf[2] = {0};
	unsigned char tries = 0;
	unsigned char userCtrlReg = 0x00;
	int result;
			...
			unsigned char disable_gz)
{
	...
	printk("@@:%d,%d,%d,%d,%d \n",reset,sleep,disable_gx,disable_gy,disable_gz);


	result = MLSLSerialRead(mlsl_handle, pdata->addr, MPUREG_PWR_MGM, 1, &b);
	ERROR_CHECK(result);
		...
		bool resume_accel, bool resume_compass)
{
	...
	/* Configure the MPU */
#if 0 /*We don't think the part necessary so that this code is skipped.*/
	result = MLSLSerialWriteSingle(mlsl_handle, mldl_cfg->addr,
					MPUREG_INT_CFG,
					(mldl_cfg->
...
#if 0
#define MEM_LOAD_CHUNK_SIZE (MPU_MEM_BANK_SIZE/8)  // customizeble chunk size must be an exact fraction of MPU_MEM_BANK_SIZE

	for (ii = 0; ii < MPU_MEM_NUM_RAM_BANKS; ii++){
		unsigned char read[MEM_LOAD_CHUNK_SIZE];
		unsigned short bytesWritten = 0;

		while (bytesWritten<MPU_MEM_BANK_SIZE) {
			result = MLSLSerialWriteMem(mlsl_handle, mldl_cfg->addr,
										((ii<<8) | (bytesWritten&0xff)),
										MEM_LOAD_CHUNK_SIZE,
										&mldl_cfg->ram[ii][bytesWritten]);
			ERROR_CHECK(result);
		//	msleep(1);
			...
			result = MLSLSerialReadMem(mlsl_handle, mldl_cfg->addr,
										...
										read);
			ERROR_CHECK(result);
			for (jj = 0; jj < MEM_LOAD_CHUNK_SIZE; jj++) {
				/* skip the register memory locations */
				if (ii == 0 && (bytesWritten+jj)<20)
					continue;
				if (mldl_cfg->ram[ii][bytesWritten+jj] != read[jj]) {
					result = ML_ERROR_SERIAL_WRITE;
					break;
				}
			}
			ERROR_CHECK(result);
			bytesWritten += MEM_LOAD_CHUNK_SIZE;
		}
    //	msleep(10);
	}
		...
		bool accel, bool compass)
{
	...
	if (ML_SUCCESS == result && accel && mldl_cfg->accel && mldl_cfg->accel->suspend) {
		result = mldl_cfg->accel->suspend(accel_handle,
						mldl_cfg->accel,
						&mldl_cfg->pdata->accel);
	}

	if (ML_SUCCESS == result && compass && mldl_cfg->compass && mldl_cfg->compass->suspend) {
 ...
 *  @return ML_SUCCESS if successful, a non-zero error code otherwise.
 */
int mpu3050_read_accel(struct mldl_cfg *mldl_cfg, void *mlsl_handle, unsigned char *data)
{
