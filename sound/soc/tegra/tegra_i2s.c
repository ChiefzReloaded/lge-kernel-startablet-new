			...
			void *dma_data)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		req->dest_wrap = 4;
		req->source_wrap = 0;
		if (info->bit_fmt == I2S_BIT_FORMAT_DSP)
				...
				struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_FORMAT_S16_LE:
		info->bit_size = I2S_BIT_SIZE_16;
		break;
	case SNDRV_PCM_FORMAT_S24_LE:
		info->bit_size = I2S_BIT_SIZE_24;
		break;
	case SNDRV_PCM_FORMAT_S32_LE:
		info->bit_size = I2S_BIT_SIZE_32;
		break;
	default:
		ret =-EINVAL;
		goto err;
	...
	i2s_set_bit_size(i2s_id, info->bit_size);
					...
					unsigned int fmt)
{
	...
	i2s_set_master(i2s_id, val);

	switch (fmt & SND_SOC_DAIFMT_FORMAT_MASK) {
	case SND_SOC_DAIFMT_DSP_A:
		info->bit_fmt = I2S_BIT_FORMAT_DSP;
		val = 0;
		break;
	...
	case SND_SOC_DAIFMT_I2S:
		info->bit_fmt = I2S_BIT_FORMAT_I2S;
		val = 0;
		break;
	case SND_SOC_DAIFMT_RIGHT_J:
		info->bit_fmt = I2S_BIT_FORMAT_RJM;
		val = 0;
		break;
	case SND_SOC_DAIFMT_LEFT_J:
		info->bit_fmt = I2S_BIT_FORMAT_LJM;
		val = 0;
		break;
	default:
		return -EINVAL;
	...
	i2s_set_bit_format(i2s_id, info->bit_fmt);
	i2s_set_left_right_control_polarity(i2s_id, val);
...
{
	struct tegra_i2s_info* info = cpu_dai->private_data;
	struct clk *dap_mclk_p = clk_get_parent(info->dap_mclk);
	struct clk *audio_clk = i2s_get_clock_by_name("audio");
	int ret = 0;

	if (clk_id == TEGRA_I2S_MCLK) {
		if (info && info->dap_mclk) {
			if (freq == DAP_MCLK_1) {
				clk_set_rate(dap_mclk_p, DAP_MCLK_1_PARENT);
			} else if (freq == DAP_MCLK_2) {
				clk_set_rate(dap_mclk_p, DAP_MCLK_2_PARENT);
			}
			clk_set_rate(info->dap_mclk, freq);
			if (clk_enable(info->dap_mclk)) {
				pr_err("%s: failed to enable dap mclk\n",
					__func__);
				ret = -EIO;
			}
		} else {
			pr_err("%s: could not get dap mclk\n", __func__);
			ret = -EIO;
			goto out;
		}
	} else if (clk_id == TEGRA_I2S_BCLK) {
		if (info && info->i2s_clk) {
			clk_set_rate(info->i2s_clk, freq);
			if (clk_enable(info->i2s_clk)) {
				pr_err("%s: failed to enable i2s-%d clock\n",
					__func__, cpu_dai->id + 1);
				ret = -EIO;
				goto out;
			}
		} else {
		...
		pr_err("%s: invalid clk id %d\n", __func__, clk_id);
		ret = -EINVAL;
	}

	pr_info("%s: dap_mclk_p=%lu, dap_mclk=%lu, dap_clk=%lu, i2s_clk=%lu, "
		"audio_sync_clk=%lu, audio=%lu\n", __func__,
		clk_get_rate(dap_mclk_p), clk_get_rate(info->dap_mclk),
		clk_get_rate(info->dap_clk), clk_get_rate(info->i2s_clk),
		clk_get_rate(info->audio_sync_clk), clk_get_rate(audio_clk));
...
static int i2s_configure(struct tegra_i2s_info *info)
{
	...
	info->bit_fmt = pdata->mode;
	info->bit_size = pdata->bit_size;
	info->fifo_fmt = pdata->fifo_fmt;
	...
	if (master && master_clk) {
		...
		if (info->bit_fmt == I2S_BIT_FORMAT_DSP)
			rate *= 2;
		i2s_set_channel_bit_count(i2s_id, master_clk, rate);
	}
	...
	i2s_set_bit_format(i2s_id, info->bit_fmt);
	i2s_set_bit_size(i2s_id, info->bit_size);
	i2s_set_fifo_format(i2s_id, info->fifo_fmt);
...
static int tegra_i2s_driver_probe(struct platform_device *pdev)
{
	...
	info->dap_mclk = i2s_get_clock_by_name(info->pdata->dap_mclk);
	if (IS_ERR(info->dap_mclk)) {
		err = PTR_ERR(info->dap_mclk);
		goto fail_unmap_mem;
	}

	info->dap_clk = i2s_get_clock_by_name(info->pdata->dap_clk);
	if (IS_ERR(info->dap_clk)) {
		err = PTR_ERR(info->dap_clk);
		goto fail_unmap_mem;
	}
